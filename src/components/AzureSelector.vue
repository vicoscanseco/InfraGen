...
const generateBicep = () => {
  try {
    ...
    content += '@description(\'Nombre del Resource Group\')\n'
...
\`\`\`bash
${deploymentCommands.value || `# 1. Validar despliegue\naz deployment sub what-if --location ${locationLabel} --template-file ${sanitizedApp}.bicep\n\n# 2. Ejecutar despliegue\naz deployment sub create --location ${locationLabel} --template-file ${sanitizedApp}.bicep`}
\`\`\`
...
const buildDeploymentCommands = (groupName, deployLocation) => {
  ...
  let commands = '# 1. Validar despliegue (What-If)\n'
  commands += `az deployment sub what-if --location ${deployLocation} --template-file main.bicep\n\n`
  commands += '# 2. Ejecutar despliegue\n'
  commands += `az deployment sub create --location ${deployLocation} --template-file main.bicep`
